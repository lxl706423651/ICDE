...
def count_communities(filename):
    """
    读取 twitter_mem.txt 文件，统计每个社区的节点数并按降序排序。
    """
...
def check(community_ids,community_list):
    node_community=[]
    for com in community_ids:
        if com in community_list:
            node_community.append(com)
    return node_community
filename = "./dataset/"+'twitter'+'/'+'twitter'+"_mem.txt"
result = count_communities(filename)
community_list=[x[0] for x in result[:28]]
...
for node, community_ids in community_dict.items():
    if G.has_node(node):  # 检查节点是否存在于图中
        community_ids=check(community_ids,community_list)
