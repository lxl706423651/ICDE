...
ccs=0.1
path='wiki'
file_path = r"./dataset/wiki/wiki-topcats-categories.txt"
...
for line in lines:
    ...
    if line.startswith("Category:"):
        parts = line.split(";")
        topic_name = parts[0].replace("Category:", "").strip()  # 去掉 'Category:' 并去空格
        node_ids = parts[1].strip().split()  # 获取节点ID并去除空格
        num_nodes = len(node_ids)  # 计算节点数量
        
        # 将话题名和节点数量存入字典
        topic_node_count[topic_name] = num_nodes

# 将字典转换为 DataFrame
topic_df = pd.DataFrame(list(topic_node_count.items()), columns=['Topic', 'Node Count'])

# 按节点数量排序并选择前 50 个话题
top_50_topics = topic_df.sort_values(by='Node Count', ascending=False).head(com_num)

# 获取前 50 个话题的名称列表
top_50_topic_names = top_50_topics['Topic'].tolist()

# 创建一个字典来存储保留的节点数据
filtered_data = []

# 重新读取文件并根据前 50 个话题筛选数据
for line in lines:
    if line.startswith("Category:"):
        parts = line.split(";")
        topic_name = parts[0].replace("Category:", "").strip()
        
        # 如果话题在前 50 个话题中，保留这行数据
        if topic_name in top_50_topic_names:
            filtered_data.append(line)
topic_node_dict = {}

node_set=set()
# 遍历 filtered_data，提取每行话题和节点
for line in filtered_data:
    if line.startswith("Category:"):
        # 去除 "Category:" 并拆分话题名称和节点
        parts = line.split(";")
        topic_name = parts[0].replace("Category:", "").strip()  # 去掉 "Category:" 并去空格
        node_ids = parts[1].strip().split()  # 获取节点ID并去除空格
        
        # 将话题和节点列表存入字典
        topic_node_dict[topic_name] = [int(node_id) for node_id in node_ids]  # 转为整数类型的节点ID
        node_set.update(topic_node_dict[topic_name])
        
datasets=[]
f=open("./dataset/wiki/wiki-topcats.txt")
data=f.read()
f.close()
rows=data.split('\n')
for row in rows:
...
for n in G.nodes():
    G.nodes[n]['community']=[]
for topic_name,node_ids in topic_node_dict.items():
    for node in node_ids:
        G.nodes[node]['community'].append(topic_name)

community_list=top_50_topic_names
...
with open("./dataset/"+path+'/'+"G.pkl", "wb") as f:
    pickle.dump(G, f)  # 将数据序列化后写入文件
with open("./dataset/"+path+'/'+"community_list.pkl", "wb") as f:
    pickle.dump(community_list, f)  # 将数据序列化后写入文件
print('community+ori_graph cost time:',time.time()-t)
...
G1=update_weight(G,community_list)#G1是F=所有topic
t=time.time()
pf = {}
pr = {}

for s in sh:
    result=find_two_hop_paths_with_max_probability(G1,s)
    if result == None: continue
    pf=pf_path(result,s,pf)

# reverse_graph = G1.reverse()
# for s in sh:
#     result=find_two_hop_paths_with_max_probability(reverse_graph,s)
#     if result == None: continue
#     pr=pf_path(result,s,pr)

with open("./dataset/"+path+'/'+"pf.pkl", "wb") as f:
    pickle.dump(pf, f)  # 将数据序列化后写入文件
with open("./dataset/"+path+'/'+"pr.pkl", "wb") as f:
    pickle.dump(pr, f)  # 将数据序列化后写入文件

print('find_two_hop_paths_with_max_probability',time.time()-t)
# del reverse_graph
# gc.collect()
...
for node in sh:
    If[node]={}
    for key,value in pf[node].items():
        x0=0
        flag=len(value)
        temp_dict={}
        if flag==1: x0=get_forward_probability(G,value[0])
        elif flag==2: 
            x0=1-(1-get_forward_probability(G,value[0]))*(1-get_forward_probability(G,value[1]))
        for i in community_list:
            # 里面其实还可以存一个标识 表示是只有一条路径得到的概率，还是2条路径得到的概率（后续可以采用不同的计算方法）
            if flag==1: temp_dict[i]=get_forward_probability(graph[i],value[0])
            elif flag==2: 
                w=1-(1-get_forward_probability(graph[i],value[0]))*(1-get_forward_probability(graph[i],value[1]))
                temp_dict[i]=w
        If[node][key]=[flag,x0,temp_dict]
...
# for node in sh:
#     Ir[node]={}
#     for nbr in G.predecessors(node): # 得到当前节点的邻居节点
#         x0 = G.get_edge_data(nbr,node)
#         flag=1
#         temp_dict={}
#         for i in community_list:
#             temp_dict[i]=graph[i].get_edge_data(nbr,node)
#         Ir[node][nbr]=[flag,x0,temp_dict]

del pr 
gc.collect

with open("./dataset/"+path+'/'+"Ir.pkl", "wb") as f:
    pickle.dump(Ir, f)  # 将数据序列化后写入文件
with open("./dataset/"+path+'/'+"If.pkl", "wb") as f:
    pickle.dump(If, f)  # 将数据序列化后写入文件
print('Ir If cost time:',time.time()-t)
...
#通过If,Ir中不受topic影响的概率值也就是x0部分将其存储成ori_forward_label,ori_reverse_label来给出一个排序
...
ori_forward_label={}
for key1 in If.keys():
    ori_forward_label[key1]={}
    for key2 in If[key1].keys():
        ori_forward_label[key1][key2]=If[key1][key2][1]

# ori_reverse_label={}
# for key1 in Ir.keys():
#     ori_reverse_label[key1]={}
#     for key2 in Ir[key1].keys():
#         ori_reverse_label[key1][key2]=Ir[key1][key2][1]
# ori_reverse_label=reverse_dict(ori_reverse_label)

ori_yxl={}
for node in node_list:
    temp={}
    if node in ori_forward_label.keys():
        for key,value in ori_forward_label[node].items():
            if key in temp.keys(): temp[key]=max(value,temp[key])
            else:temp[key]=value
            if key in ori_forward_label.keys():
                for key1,value1 in ori_forward_label[key].items():
    ...
    ori_yxl[node]=sum(temp.values())
sorted_list = sorted(ori_yxl.items(), key=lambda x: x[1], reverse=True)
with open("./dataset/"+path+'/'+"sorted_list.pkl", "wb") as f:
    pickle.dump(sorted_list, f)  # 将数据序列化后写入文件
print('sorted_list cost time:',time.time()-t22)
